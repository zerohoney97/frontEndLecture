...
import { IBlock } from "@coreP2P/interface/block.interface";
import { Failable } from "@coreP2P/interface/failable.interface";
import CryptoModule from "@coreP2P/crypto/crypto.module";
...
class Block extends BlockHeader implements IBlock {
  ...
  static getDifficulty(
    ...
    // 2016개 이전 블록의 생성시간을 보고 난이도 조절
    // 우리는 10개 이전
    if (_newBlock.height <= 0) {
      throw new Error("높이가 0이 들어왔다.");
    }
    if (_newBlock.height < 10) return 0;
    if (_newBlock.height < 21) return 1;

    // 블록의 높이가 20이하일 경우에는 체크 x
    // 블록의 높이가 10의 배수가 아닐 경우에는 이전 블록 난이도 설정
    ...
    // 난이도 증가
    // 생성시간이 빨랐니 총걸린시간<목표시간/2=이전 블록 난이도 +1

    if (timeToken < timeExpected / 2) {
      return _previousBlock.difficulty + 1;
    }

    // 난이도 감소
    // 생성시간이 더 걸렸다 총걸린 시간>목표시간 *2=이전블록 난이도 -1
    if (timeToken > timeExpected * 2) {
      return _previousBlock.difficulty - 1;
    }
